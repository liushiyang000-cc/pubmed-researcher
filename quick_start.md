## Quick Earning Checklist

### TODAY (Do These First)

1. Fiverr - Upload 3 gigs
   - Gig 1: Medical Literature Review (-25-80)
   - Gig 2: SCI Paper Editing (-30-100)
   - Gig 3: PubMed Data Extraction (-20-50)
   Portfolio images: portfolio_project1/2/3.png
   Gig covers: fiverr_gig1/2/3_cover.png
   Location: D:/github/pubmed-researcher/

2. Upwork - Create profile
   Copy content from UPWORK_PROFILE.md
   https://www.upwork.com/signup

3. Social Media - Start promoting
   - LinkedIn: Post article from LINKEDIN_ARTICLE.md
   - Twitter: Post Day 1 from TWEETS.md
   - Reddit: Post in r/medicalschool from REDDIT_POSTS.md

### THIS WEEK

- Apply to 5-10 Upwork jobs daily
- Post on Fiverr Gigs promotion
- Respond to any Fiverr messages quickly
- Build GitHub Stars (share on Twitter/Reddit)

### PASSIVE INCOME (Set and Forget)

- GitHub Sponsors: Waiting for approval
- Gumroad: Upload products from GUMROAD_PRODUCTS.md
- Substack: Start newsletter (long-term)
- Online Course: Record PubMed Research Guide (Udemy)

### EARNING POTENTIAL BY PLATFORM

| Platform | Time to First | Monthly Potential |
|----------|---------------|-------------------|
| Fiverr | 1-2 weeks | $200-$800 |
| Upwork | 1-3 weeks | -2000 |
| GitHub Sponsors | 1-2 months | -200 |
| Gumroad | 2-4 weeks | -500 |
| Substack | 3-6 months | -1000 |
| Online Course | 2-3 months | -2000 |

### IMPORTANT: Fix GitHub Token

Your token was exposed in chat.
Immediately revoke at: https://github.com/settings/tokens
Then regenerate and update .git/config
